...
io.on("connection", (soc) => {
  console.log("There is a new connection");
  soc.on("message", async (msg) => {
    console.time("prompt-response-time");
    console.log(`Received message: ${msg}`);
    ...
    console.timeEnd("prompt-response-time");
  });
});

// Serve static files from the React app
app.use(express.static(path.join(__dirname, "GPTReact/dist")));

app.get("*", (req, res) => {
  res.sendFile(path.join(__dirname, "GPTReact/dist", "index.html"));
});
//C:\repos\ChatGPTCloneIndu - Tester\GPTReact\dist\index.html
